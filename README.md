...
- **贴图绑定 / Hash 贴图节点**：支持连接物体的条件替换，以及不连接任何节点的全局 Hash 替换。
  - **Hash 贴图绑定 / 全局 Hash 贴图绑定**：共用“刷新检测 → 原图/替换图并排预览 → 选择替换贴图”的布局，不需要手填 Hash、选择来源类型或手动增删行。只有一个文件选择按钮，× 用于清除已有替换图。
  - 普通 Hash 节点只检测上游连线涉及的 Submesh，遵循对象列表及节点组的具体输出端口；全局节点检测整个工作空间。相同 Hash 合并为一条。普通节点仍需接入导出链，并保留开关条件。
  - 标题、按钮和提示随插件的“Language / 语言”设置切换；旧的默认节点标题也会更新，自定义标题保持不变。
  - 刷新保留已指定的替换图；未指定替换图、清除替换路径或禁用的条目不生成额外覆盖。曾经指定过替换图的条目，在清除或禁用后再次生成 Mod 时恢复原标记图的文件内容。已指定替换图的标记如果消失，会保留条目并提示处理，避免静默丢失配置。预览不支持的图片显示占位提示，不影响文件选择。
  - 生成 Mod 时复制替换文件；无条件替换的 DDS 保留 `<Hash>_<标记名>.dds` 命名，其他支持格式保留实际扩展名。带开关条件的替换使用独立资源文件，避免不同状态的贴图相互覆盖。
- **形态键节点**：按按键切换形态键数值。
