...
- **贴图绑定 / Hash 贴图节点**：支持连接物体的条件替换，以及不连接任何节点的全局 Hash 替换。
  - **全局 Hash 贴图绑定**：点击“刷新检测Hash标记贴图”，自动列出工作空间的 Hash 标记（同一 Hash 合并为一条），无需手填 Hash 或选择来源类型。选择列表条目后，并排查看原标记贴图与替换贴图，用文件浏览按钮指定替换图。
  ...
  - 生成 Mod 时复制替换文件；DDS 保留 `<Hash>_<标记名>.dds` 命名，其他支持格式保留实际扩展名。
- **形态键节点**：按按键切换形态键数值。
