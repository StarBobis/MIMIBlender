...
- **贴图绑定 / Hash 贴图节点**：Slot 贴图节点按物体绑定贴图槽；Hash 贴图节点分全局与按物体两种替换。
  - **Hash 贴图绑定 / 全局 Hash 贴图绑定**：共用“刷新检测 → 原图/替换图并排预览 → 选择替换贴图”的布局，不需要手填 Hash、选择来源类型或手动增删行。只有一个文件选择按钮，× 用于清除已有替换图。
  - 普通 Hash 节点只检测上游连线涉及的 Submesh，遵循对象列表及节点组的具体输出端口；全局节点检测整个工作空间。相同 Hash 合并为一条。普通节点仍需接入导出链。
  - 同一物体同一个贴图槽（如 ps-t0）同时被 Slot 贴图节点和 Hash 节点绑定时会直接报错，避免静默地二选一。
  - 标题、按钮和提示随插件的“Language / 语言”设置切换；旧的默认节点标题也会更新，自定义标题保持不变。
  - 刷新保留已指定的替换图；未指定替换图的条目不生成覆盖。普通节点上清除或禁用某条，该物体就回退到全局替换图。已指定替换图的标记如果消失，会保留条目并提示处理，避免静默丢失配置。预览不支持的图片显示占位提示，不影响文件选择。
  - **替换范围与优先级**：全局节点的贴图是该 Hash 的**默认值**；普通节点的贴图**只作用于它连接的物体**（绑定写在物体自己的 draw call 之前）。所以同一个 Hash 只生成**一个** `[TextureOverride]` 段，避免 3Dmigoto 报 duplicate hash override，也不需要靠 `match_priority` 在两段之间争优先级。
  - **有开关用开关、没开关自动按物体**：物体经过 Switch Key 时，该绑定自动落在对应状态的 `if` 块里；没有开关时按物体本身生效（与 Slot 贴图节点同一套机制，所有游戏预设通用）。未命中任何状态的物体继续使用全局替换图。
  - 生成 Mod 时复制替换文件；全局替换的 DDS 保留 `<Hash>_<标记名>.dds` 命名，按物体替换使用独立文件名（同名冲突时自动追加序号），其他支持格式保留实际扩展名。因此全局图与物体图不会再互相覆盖。
- **形态键节点**：按按键切换形态键数值。
