...
- **Blueprint node editor**: visual node graphs for mod logic — object / object-list, texture & hash-texture bind, shape key, time-switch animation nodes (loop or key-triggered, with baking), face-mod export, groups, result outputs, file drag-and-drop, Naraka-specific nodes.
