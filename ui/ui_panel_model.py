...
def keep_one_triangle_in_mesh_object(obj):
    if obj.type != 'MESH':
        raise ValueError(tr("Selected object is not a mesh."))

    mesh = obj.data
    if len(mesh.polygons) == 0:
        raise ValueError(obj.name + tr(" has no faces."))
    ...
    if tri_face is None:
        bm.free()
        raise ValueError(obj.name + tr(" has no triangle face."))
...
class ModelSplitByLoosePart(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Split Model by UV Loose Parts Success!"))
        return {'FINISHED'}
...
class ModelSplitByVertexGroup(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Split Model by Vertex Group Success!"))
        return {'FINISHED'}
...
class ModelDeleteLoosePoint(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Delete Loose Points Success!"))
        return {'FINISHED'}
...
class ModelClearCustomSplitNormals(I18nOperator):
    ...
    def execute(self, context):
        ...
        if not sel:
            self.report({'ERROR'}, tr("No object selected."))
            return {'CANCELLED'}
        for obj in sel:
            if obj.type == 'MESH':
...
class KeepOneTriangleInSelectedSubmesh(I18nOperator):
    ...
    def execute(self, context):
        ...
        if not mesh_objects:
            self.report({'ERROR'}, tr("No Mesh/Submesh objects selected."))
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, tr("Reduced {count} Submesh(es) to a single triangle face").format(count=processed_count))
        return {'FINISHED'}
...
class ModelRenameVertexGroupNameWithTheirSuffix(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Rename Vertex Groups with Model Name Prefix Success!"))
        return {'FINISHED'}
...
class RemoveAllVertexGroupOperator(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Remove All Vertex Groups Success!"))
        return {'FINISHED'}
...
class RemoveUnusedVertexGroupOperator(I18nOperator):
    ...
    def execute(self, context):
        ...
        for obj in bpy.context.selected_objects:
            VertexGroupUtils.remove_unused_vertex_groups(obj)
        self.report({'INFO'}, tr("Remove Unused Empty Vertex Groups Success!"))
        return {'FINISHED'}
...
class MergeVertexGroupsWithSameNumber(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Merge Vertex Groups with the Same Numeric Prefix") + tr(" Success!"))
        return {'FINISHED'}
...
class FillVertexGroupGaps(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Fill Numeric Vertex Group Gaps") + tr(" Success!"))
        return {'FINISHED'}
...
class AddBoneFromVertexGroupV2(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Generate Basic Bones from Vertex Groups") + tr(" Success!"))
        return {'FINISHED'}
...
class RemoveNotNumberVertexGroup(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Remove Non-Numeric Vertex Groups") + tr(" Success!"))
        return {'FINISHED'}
...
class SplitMeshByCommonVertexGroup(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Break Model into Loose Parts by Vertex Groups") + tr(" Success!"))
        return {'FINISHED'}
...
class SplitMeshByEachVertexGroup(I18nOperator):
    ...
    def execute(self, context):
        ...
        obj = bpy.context.selected_objects[0]
        if obj.type != 'MESH':
            self.report({'ERROR'}, tr("The selected object is not a mesh!"))
            return {'CANCELLED'}
        try:
            collection = VertexGroupUtils.split_mesh_by_each_vertex_group(obj)
            self.report({'INFO'}, tr("Split into independent meshes by vertex group, {count} objects in total").format(count=len(collection.objects)))
        except Exception as e:
...
class SplitMeshByEachVertexGroupCluster(I18nOperator):
    ...
    vg_similarity_threshold: bpy.props.FloatProperty(
        name=tr("VG Similarity Threshold"),
        description=tr("Jaccard similarity (intersection/union); two loose parts are merged when the similarity of their VG sets is >= this value and they are spatially adjacent"),
        default=0.7,
        min=0.1,
        max=1.0,
    ) # type: ignore
    ...
    bbox_distance_threshold: bpy.props.FloatProperty(
        name=tr("BBox Distance Threshold"),
        description=tr("Two loose parts are considered spatially adjacent when their bounding-box distance is <= this value"),
        default=0.01,
        min=0.0001,
        soft_max=1.0,
    ) # type: ignore
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'INFO'}, tr("Split by loose parts and clustered, {count} objects in total").format(count=len(collection.objects)))
        except Exception as e:
            self.report({'ERROR'}, str(e))
            return {'CANCELLED'}
        return {'FINISHED'}
...
class MMTResetRotation(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Reset Model Rotation on X, Y, Z to 0") + tr(" Success!"))
        return {'FINISHED'}
...
class WWMI_ApplyModifierForObjectWithShapeKeysOperator(I18nOperator):
    ...
    disable_armatures: BoolProperty(
        name=tr("Exclude Armature Deformation"),
        default=True,
    ) # type: ignore
    ...
    def execute(self, context):
        ...
        if not selectedModifiers:
            self.report({'ERROR'}, tr('No modifiers selected!'))
            return {'FINISHED'}
    ...
    def draw(self, context):
        if context.object.data.shape_keys and context.object.data.shape_keys.animation_data:
            ...
            self.layout.label(text=tr("Warning:"))
            self.layout.label(text=tr("The shape keys of this object contain animation data"))
            self.layout.label(text=tr("(e.g. drivers, keyframes, etc.)"))
            self.layout.label(text=tr("These data will be lost after the modifiers are applied!"))
            self.layout.separator()
        ...
        self.layout.prop(self, "disable_armatures", text=tr("Exclude Armature Deformation"))
    ...
    def execute(self, context):
        for obj in bpy.context.selected_objects:
            if obj.type == "MESH":
                ...
                self.report({'INFO'}, tr("Recalculate TANGENT set to: ") + str(obj["3DMigoto:RecalculateTANGENT"]))
        return {'FINISHED'}
    ...
    def execute(self, context):
        for obj in bpy.context.selected_objects:
            if obj.type == "MESH":
                ...
                self.report({'INFO'}, tr("Recalculate COLOR set to: ") + str(obj["3DMigoto:RecalculateCOLOR"]))
        return {'FINISHED'}
...
class RenameAmatureFromGame(I18nOperator):
    ...
    def execute(self, context):
        ...
        if not bpy.context.active_object:
            raise RuntimeError(tr("The selected object is not an armature."))
        if bpy.context.active_object.type != "ARMATURE" or armature_name not in bpy.data.objects:
            raise RuntimeError(tr("Error: No object selected."))
...
class ModelResetLocation(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Reset Model Location on X, Y, Z to 0") + tr(" Success!"))
        return {'FINISHED'}
...
class ModelSortVertexGroupByName(I18nOperator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, tr("Sort Vertex Groups by Name") + tr(" Success!"))
        return {'FINISHED'}
    
class ModelVertexGroupRenameByLocation(I18nOperator):
    ...
    def execute(self, context):
        if len(bpy.context.selected_objects) < 2:
            self.report({'ERROR'}, tr("Not enough objs selected! Select the source obj first, then the target obj. Usually the target obj is your own model and the source obj is the game's original model"))
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, tr("Rename Target Object Vertex Groups by Positional Mapping") + tr(" Success!"))
...
class ExtractSubmeshOperator(I18nOperator):
    ...
    start_index: bpy.props.IntProperty(
        name=tr("Start Index"),
        description=tr("Start index inside the index buffer"),
        default=0,
        min=0
    ) # type: ignore
    ...
    index_count: bpy.props.IntProperty(
        name=tr("Index Count"),
        description=tr("Number of indices to take; must be a multiple of 3"),
        default=3,
        min=3
    ) # type: ignore
    ...
    def execute(self, context):
        ...
        if not obj or obj.type != 'MESH':
            self.report({'ERROR'}, tr("Please select a mesh object"))
            return {'CANCELLED'}
        ...
        if start + count > len(original_mesh.loops):
            self.report({'ERROR'}, tr("Index range exceeds the buffer; maximum loop count is: {count}").format(count=len(original_mesh.loops)))
            return {'CANCELLED'}
            
        if count % 3 != 0:
            self.report({'ERROR'}, tr("Index count must be a multiple of 3"))
            return {'CANCELLED'}
...
# Dynamic enum item callbacks for the mirror operator: translating the display
# name/description while keeping the identifiers stable, so the redo panel (F9)
# follows the selected UI language without breaking saved values.
def _get_mirror_mode_items(self, context):
    return [
        ("COPY", tr("Mirrored Copy"),
         tr("Keep every selected object and create a clean mirrored copy next to it")),
        ("FLIP", tr("Flip In Place"),
         tr("Mirror every selected object in place; click again to swap the side back")),
        ("BAKE", tr("Bake & Fix"),
         tr("Fix an old object that was already mirrored with a negative scale: bake the scale "
            "into the mesh, repair the normals, reset the scale to 1, but keep the current look")),
    ]


def _get_mirror_axis_items(self, context):
    return [
        ("X", tr("X Axis"), tr("Mirror left / right")),
        ("Y", tr("Y Axis"), tr("Mirror front / back")),
        ("Z", tr("Z Axis"), tr("Mirror top / bottom")),
    ]


def _get_mirror_uv_items(self, context):
    return [
        ("NONE", tr("None"), tr("Keep UV maps as they are (mirror image look)")),
        ("U", tr("Flip U"), tr("u becomes 1 - u, texture direction is kept readable")),
        ("V", tr("Flip V"), tr("v becomes 1 - v, texture direction is kept readable")),
    ]


class MirrorMeshOperator(I18nOperator):
    ...
    mode: EnumProperty(
        name=tr("Mode"),
        description=tr("What to do with every selected mesh object"),
        # Dynamic items only allow integer (0-based) defaults; the first item
        # ("COPY") is the intended default, so the argument is omitted.
        items=_get_mirror_mode_items,
    ) # type: ignore

    axis: EnumProperty(
        name=tr("Mirror Axis"),
        description=tr("The mirror plane is the axis plane through the object origin "
                       "(the same plane a negative scale on that axis would use)"),
        items=_get_mirror_axis_items,
    ) # type: ignore

    recalc_normals: BoolProperty(
        name=tr("Recalculate Normals (Outward)"),
        description=tr("Also recalculate all normals to point outside, which repairs faces "
    ...
    mirror_uv: EnumProperty(
        name=tr("Mirror UV"),
        description=tr("Mirror the UV maps around the middle of the UV tile. Default (None) gives "
        ...
        items=_get_mirror_uv_items,
    ) # type: ignore

    swap_side_groups: BoolProperty(
        name=tr("Swap L/R Vertex Group Names"),
        description=tr("Rename mirrored vertex groups so symmetric rigs deform the copy correctly "
    ...
    copy_suffix: StringProperty(
        name=tr("Copy Name Suffix"),
        description=tr("Name suffix of the created mirrored copies (only used in Mirrored Copy mode)"),
        default="_mirror",
    ) # type: ignore
    ...
    def execute(self, context):
        ...
        if len(mesh_objects) == 0:
            self.report({'ERROR'}, tr("Select at least one mesh object to mirror."))
            return {'CANCELLED'}
        ...
        for error in errors:
            self.report({'WARNING'}, tr("Mirror skipped - {error}").format(error=error))

        if len(mirrored_objects) > 0:
            self.report({'INFO'}, tr("Mirror success: {count} object(s).").format(count=len(mirrored_objects)))
            return {'FINISHED'}
        self.report({'ERROR'}, tr("Nothing was mirrored."))
        return {'CANCELLED'}
...
class MIMIPanelModelProcess(bpy.types.Panel):
    ...
    def draw(self, context):
        # Every operator button passes text=tr(...) so the caption follows the
        # selected UI language; bl_label stays English as the translation key.
        layout = self.layout
        layout.operator(MirrorMeshOperator.bl_idname, text=tr(MirrorMeshOperator.bl_label))
        layout.operator(ModelResetLocation.bl_idname, text=tr(ModelResetLocation.bl_label))
        layout.operator(MMTResetRotation.bl_idname, text=tr(MMTResetRotation.bl_label))
        layout.operator(ModelDeleteLoosePoint.bl_idname, text=tr(ModelDeleteLoosePoint.bl_label))
        layout.operator(ModelClearCustomSplitNormals.bl_idname, text=tr(ModelClearCustomSplitNormals.bl_label))
        layout.operator(KeepOneTriangleInSelectedSubmesh.bl_idname, text=tr(KeepOneTriangleInSelectedSubmesh.bl_label))
        layout.separator()



        layout.operator(RemoveAllVertexGroupOperator.bl_idname, text=tr(RemoveAllVertexGroupOperator.bl_label))
        layout.operator(RemoveUnusedVertexGroupOperator.bl_idname, text=tr(RemoveUnusedVertexGroupOperator.bl_label))
        layout.operator(RemoveNotNumberVertexGroup.bl_idname, text=tr(RemoveNotNumberVertexGroup.bl_label))
        layout.separator()

        layout.operator(ModelSortVertexGroupByName.bl_idname, text=tr(ModelSortVertexGroupByName.bl_label))
        layout.operator(FillVertexGroupGaps.bl_idname, text=tr(FillVertexGroupGaps.bl_label))
        layout.operator(MergeVertexGroupsWithSameNumber.bl_idname, text=tr(MergeVertexGroupsWithSameNumber.bl_label))
        layout.operator(ModelVertexGroupRenameByLocation.bl_idname, text=tr(ModelVertexGroupRenameByLocation.bl_label))
        layout.separator()

        layout.operator(ModelRenameVertexGroupNameWithTheirSuffix.bl_idname, text=tr(ModelRenameVertexGroupNameWithTheirSuffix.bl_label))
        layout.operator(AddBoneFromVertexGroupV2.bl_idname, text=tr(AddBoneFromVertexGroupV2.bl_label))
        layout.separator()

        layout.operator(WWMI_ApplyModifierForObjectWithShapeKeysOperator.bl_idname, text=tr(WWMI_ApplyModifierForObjectWithShapeKeysOperator.bl_label))
        layout.operator(SmoothNormalSaveToUV.bl_idname, text=tr(SmoothNormalSaveToUV.bl_label))
        layout.operator(RenameAmatureFromGame.bl_idname, text=tr(RenameAmatureFromGame.bl_label))
        layout.separator()
        
        layout.operator(RecalculateTANGENTWithVectorNormalizedNormal.bl_idname, text=tr(RecalculateTANGENTWithVectorNormalizedNormal.bl_label))
        layout.operator(RecalculateCOLORWithVectorNormalizedNormal.bl_idname, text=tr(RecalculateCOLORWithVectorNormalizedNormal.bl_label))
        layout.separator()
        
        layout.operator(ModelSplitByLoosePart.bl_idname, text=tr(ModelSplitByLoosePart.bl_label))
        layout.operator(SplitMeshByCommonVertexGroup.bl_idname, text=tr(SplitMeshByCommonVertexGroup.bl_label))
        layout.operator(ModelSplitByVertexGroup.bl_idname, text=tr(ModelSplitByVertexGroup.bl_label))
        layout.operator(SplitMeshByEachVertexGroup.bl_idname, text=tr(SplitMeshByEachVertexGroup.bl_label))
        layout.operator(SplitMeshByEachVertexGroupCluster.bl_idname, text=tr(SplitMeshByEachVertexGroupCluster.bl_label))
...
class MIMICatterRightClickMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        # Right-click menu copy of the panel buttons, with translated captions.
        layout = self.layout
        layout.operator(MirrorMeshOperator.bl_idname, text=tr(MirrorMeshOperator.bl_label))
        layout.operator(ModelResetLocation.bl_idname, text=tr(ModelResetLocation.bl_label))
        layout.operator(MMTResetRotation.bl_idname, text=tr(MMTResetRotation.bl_label))
