...
def gimi_face_local_to_game_positions(positions: numpy.ndarray) -> numpy.ndarray:
    """Undo MMT's applied GIMI import rotation for face-buffer positions.
...
def build_face_ini(parts: Iterable[FaceModPart], diffuse_hash: str = "") -> str:
    ...
    lines: list[str] = ["; Generated by MIMIBlender MMT face-mod exporter.", ""]
    ...
    for part, part_name in named_parts:
        ...
        for index_hash in parse_face_index_hashes(part.index_hashes):
            lines.extend([
                f"[TextureOverrideMMTFace{part_name}_{index_hash}]",
                f"hash = {index_hash}",
            ])
            if gate_hash:
                lines.extend([f"if {gate_variable}", f"  run = CommandListMMTFace{part_name}", "endif"])
            else:
                lines.append(f"run = CommandListMMTFace{part_name}")
            lines.append("")
        lines.extend([
            "",
            f"[CommandListMMTFace{part_name}]",
            f"ResourceMMTFace{part_name}Dif = copy vb0",
            f"run = CustomShaderMMTFace{part_name}",
            f"vb0 = ResourceMMTFace{part_name}Dif",
            "",
            f"[ResourceMMTFace{part_name}Dif]",
            "",
            f"[ResourceMMTFace{part_name}Base]",
            "type = RWBuffer",
            f"stride = {FACE_VERTEX_STRIDE}",
            f"filename = {relative_prefix}base.buf",
            "",
            f"[ResourceMMTFace{part_name}Key]",
            "type = RWBuffer",
            f"stride = {FACE_VERTEX_STRIDE}",
            f"filename = {relative_prefix}key.buf",
            "",
            f"[CustomShaderMMTFace{part_name}]",
            "cs = Face.hlsl",
            "",
            f"cs-u1 = copy ResourceMMTFace{part_name}Dif",
            f"cs-t0 = copy ResourceMMTFace{part_name}Base",
            f"cs-t1 = copy ResourceMMTFace{part_name}Key",
            "",
            f"Dispatch = {part.vertex_count}, 1, 1",
            f"ResourceMMTFace{part_name}Dif = copy cs-u1",
            "post cs-u1 = null",
            "",
        ])
