...
class M_IniBuilder:
    ...
    def append_section(self,m_inisection:M_IniSection):
        # Constants and Present are singleton sections in a 3Dmigoto INI.
        # Older writers put literal headers in their body, while newer ones
        # use SectionName. Normalize both before concatenating contributions.
        # Never normalize arbitrary resource/command-list sections this way.
        if m_inisection.SectionType in (M_SectionType.Constants, M_SectionType.Present):
            name = m_inisection.SectionType
            header = "[" + name.lower() + "]"
            m_inisection.SectionLineList = [
                line for line in m_inisection.SectionLineList
                if line.strip().lower() != header
            ]
            m_inisection.SectionName = name
            # Merge at insertion time, not only when sorting on save. WWMI
            # deliberately preserves append order and also needs one header.
            # Contribution order inside the singleton remains unchanged.
            for existing in self.ini_section_list:
                if existing.SectionType == name:
                    existing.SectionLineList.extend(m_inisection.SectionLineList)
                    return
    ...
    def save_to_file_not_reorder(self,config_ini_path:str):
        ...
        '''
        # Rendering the same builder again must not append another INI copy.
        self.line_list.clear()
        self.ini_section_name_set.clear()
    ...
    def save_to_file(self,config_ini_path:str):
        # Keep serialization repeatable for preview, validation and final save.
        self.line_list.clear()
        self.ini_section_name_set.clear()
