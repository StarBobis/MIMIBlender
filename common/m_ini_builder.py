...
# Tail comments that XXMI Tools (leotorrez/XXMITools) writes at the end of the
# INI files it generates, taken from its templates/base.ini.j2. The version
# number of that add-on is deliberately left out. They exist so a generated mod
# can carry the same trailing lines as an XXMI Tools mod; the flag lives on the
# Generate Mod node (scene level, like the other export options there).
...
def _simulate_xxmi_tail_comments() -> bool:
    """Read the Generate Mod node option without a hard bpy dependency.

    The import stays lazy so this module keeps working in the pure Python
    tests, and any missing scene simply means "option off".
    """
    try:
        from .mimi_global_properties import MIMIGlobalProperties

        return bool(MIMIGlobalProperties.simulate_xxmi_tail_comments())
    except Exception:
        return False


def get_xxmi_tail_comment_lines() -> list[str]:
    """Return the simulated XXMI tail comments, or nothing when disabled.

    Only the comment lines themselves are returned; the blank line that
    separates them from the content is added by each writer, so the result
    matches the spacing of a real XXMI Tools file.
    """
    if not _simulate_xxmi_tail_comments():
        return []
    return list(XXMI_TAIL_COMMENT_LINES)


def append_xxmi_tail_comment_lines(line_list: list) -> list:
    """Append the tail comments to a rendered line list, in place.

    Exactly one blank line is kept in front of the comments: the writers
    already end their last section with a blank line, and duplicating it would
    not look like the original file.
    """
    tail_lines = get_xxmi_tail_comment_lines()
    if not tail_lines:
        return line_list
    if not line_list or line_list[-1].strip() != "":
        line_list.append("\n")
    for tail_line in tail_lines:
        line_list.append(tail_line + "\n")
    return line_list
...
class M_IniBuilder:
    ...
    def save_to_file_not_reorder(self,config_ini_path:str):
        ...
        # Optional tail comments that make the file end like an XXMI Tools one.
        # They are added before hashing so toggling the option rewrites the INI.
        append_xxmi_tail_comment_lines(self.line_list)

        # Add sha256 to verify if ini need to overwrite.
        sha256 = self.calculate_sha256_for_list(self.line_list)
        # print("sha256: " + sha256)

        # Add after sha256 calculation. It stays the last line of the file.
        self.line_list.append("\n;sha256=" + sha256 + "\n\n") 
